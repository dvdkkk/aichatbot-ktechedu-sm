...
export const ReviewSection: React.FC = () => {
  ...
  return (
    <section id="reviews" className="py-24 bg-black relative border-t border-zinc-900 overflow-hidden">
      {/* Background Decor */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-64 h-64 bg-yellow-400/5 rounded-full blur-[80px] pointer-events-none"></div>
        ...
        <Reveal delay={200} className="relative max-w-6xl mx-auto overflow-hidden">
          ...
          >
            {REVIEWS.map((review, idx) => (
              ...
              >
                <div className="h-full bg-zinc-900/50 border border-zinc-800 p-8 rounded-3xl relative flex flex-col hover:border-yellow-400/30 transition-colors group">
                  ...
                  <div className="flex gap-1 mb-4">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        size={16} 
                        className={`${i < Math.floor(review.score) ? 'fill-yellow-400 text-yellow-400' : (i < review.score ? 'fill-yellow-400 text-yellow-400 opacity-50' : 'text-zinc-700')}`} 
                      />
                    ))}
